        ...
        if (json.status) {
            ...
            json.data.forEach(t_venta => {
                lista_temporal += `<tr>
                                    ...
                                    <td><button class="btn btn-danger btn-sm" inclick="eliminarTerminar(${t_venta.id});">Eliminar</button></td>
                                </tr>`
            });
...
async function registrarVenta() {
    ...
    if(id_cliente == ''|| fecha_venta == ''){
        return alert("debe completar los datos del cliente y la fecha de venta");
    }
    try {
        const datos = new FormData();
        ...
        console.log("error al registra venta " + error);
    }
